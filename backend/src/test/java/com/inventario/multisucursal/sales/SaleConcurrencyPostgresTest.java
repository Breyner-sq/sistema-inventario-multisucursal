...
/**
 * Mismo escenario y mismas aserciones que {@link SaleConcurrencyTest}, pero
 * contra PostgreSQL real vía Testcontainers — ver el javadoc de
...
class SaleConcurrencyPostgresTest {
    ...
    void twoConcurrentSalesOnLastStockOnlyOneSucceeds() throws Exception {
        Branch branch = branchRepository.save(new Branch("SUC-CCPG-S", "Sucursal Concurrencia Ventas PG", null));
        User operator = userRepository.save(new User(
                "Operador CC PG", "operator.cc.sales.pg@test.local", passwordEncoder.encode("ChangeMe123!"), RoleCode.OPERATOR, branch.getId()));
        UnitOfMeasure unit = unitOfMeasureRepository.save(new UnitOfMeasure("UN-CCPG-S", "Unidad"));
        Product product = productRepository.save(new Product("SKU-CCPG-SALE-001", "Producto Concurrencia Venta PG", null, unit.getId()));
        productUnitRepository.save(new ProductUnit(product.getId(), unit.getId(), BigDecimal.ONE, true));
        PriceList priceList = priceListRepository.save(new PriceList("Lista CC PG", null));
